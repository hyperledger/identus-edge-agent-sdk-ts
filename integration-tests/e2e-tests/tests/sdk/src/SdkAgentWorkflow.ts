...
export class SdkAgentWorkflow {
  ...
  static async createAgent() {
    ...
    this.edgeAgent.addListener(ListenerKey.MESSAGE, async (messages: Domain.Message[]) => {
  ...
  static async acceptCredential(edgeAgent: Actor) {
    ...
    let requestCredential = await this.edgeAgent.prepareRequestCredentialWithIssuer(message)
    try {
      await this.edgeAgent.sendMessage(requestCredential.makeMessage())
    } catch (e) {
      console.error('This should not appear', new Error().stack?.split('\n')[1])
    }
  }
  ...
  static async presentProof(edgeAgent: Actor) {
    ...
    const requestPresentationMessage = RequestPresentation.fromMessage(this.proofRequestStack.pop()!)
    let presentation = await this.edgeAgent.createPresentationForRequestProof(requestPresentationMessage, credential)
    try {
