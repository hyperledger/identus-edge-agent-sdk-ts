...
export default class DIDCommAgent extends Startable.Controller {
  ...
  private runTask<T>(task: Task<T>) {
    ...
    // ? do we always want all plugin extensions - or should we be more precise
    // ie - only load transport layer when sending
    //   ctx.transport.send - where tranport is known interface
  ...
  /**
   * Stops all long running jobs
   * such as fetching messages
   */
