...
export class IsCredentialRevoked extends Pollux.IsCredentialRevoked {
  async run(ctx: Task.Context) {
    ...
    if (credential instanceof JWTCredential) {
      ...
      return this.verifyRevocationProof(ctx, response, revocationStatus.statusListIndex);
    }
  ...
  private async fetchRevocationRegistry(ctx: Task.Context, revocationStatus: JWTRevocationStatus) {
    ...
    // TODO validate JWTStatusListResponse body
  ...
  private async encode(ctx: Task.Context, data: any) {
    const customLoader = async (url: string) => {
      ...
      if (cached) {
        const doc: RemoteDocument = {
          documentUrl: url,
          // TODO check why these types dont match
