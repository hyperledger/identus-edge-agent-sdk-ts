...
import { PlugPol } from "../../src/pollux/PlugPol";
import DIFModule from "../../src/pollux/plugins/dif";
...
describe("Pollux", () => {
    ...
    it("Should correctly determine that a Credential is revoked when calling the credentialStatus [EcdsaSecp256k1Signature2019] list endpoints", async () => {
        ...
        const revoked = await pollux.handle("revocation", "prism/jwt", {credential: credential});
        const revoked2 = await pollux.handle("revocation", "prism/jwt", {credential: credential2});
        const revoked3 = await pollux.handle("revocation", "prism/jwt", {credential: credential3});
    ...
    it("Should throw an error if we try to use a SDJWT credential ", async () => {
      vi.spyOn(api, "request").mockResolvedValue(new ApiResponse({
        ...
        await expect(pollux.handle("revocation", "prism/jwt", {credential: credential}))
    ...
    it("Should throw an error if we try to use a credential which an unsupported type ", async () => {
        ...
        const sut2 = JWTCredential.fromJWS(revocableJWTCredential);
        await expect(pollux.handle("revocation", "prism/jwt", {credential: sut2}))
    ...
    it("Should throw an error if the credential status proof contains an invalid verificationMethod", async () => {
        ...
        const sut = JWTCredential.fromJWS(revocableJWTCredential);
        await expect(pollux.handle("revocation", "prism/jwt", {credential: sut}))
